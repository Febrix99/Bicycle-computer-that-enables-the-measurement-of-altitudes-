...
class Menu():   
    ...
    Pin18_state, Pin16_state ,zmiana_przycisk            = 0 ,0, False
    flaga_sen_stop_menu , spac_menu  ,wybor_ktory_motyw  = False, False, 0 
...
#################################### FUNKCJE PRZERWANIA ##########################################################################################    
    ...
    def Przycisk_1(self,pin):#[LEWY/ ZIELONY] Funkcja realizująca przerwanie przycisku od zmiany wyświetlacza [1]
        if (self.Przycisk_1_Pin.value() == 1) and (self.Pin16_state == 0 ):
            self.Pin16_state = 1
            ...
            self.interaction_przycisk = time.ticks_ms() # przypisanie aktualnego czasu po kliknięciu        
            ##===== Wybudzenie ze snu ========##       
            if self.spac_menu == True :
                self.flaga_sen_stop_menu = False
            #==================================#            
            
        elif (self.Przycisk_1_Pin.value() == 0) and (self.Pin16_state == 1 ):
            self.Pin16_state = 0
            
        self.Przycisk_1_Pin.irq(handler = self.Przycisk_1)    

     
    def Przycisk_2(self,pin):  # [PRAWY/ CZERWONY]Funkcja realizująca przerwanie przycisku od zmiany wyświetlacza [2]
        if (self.Przycisk_2_Pin.value() == 1) and (self.Pin18_state == 0 ):
            self.Pin18_state = 1
            micropython.alloc_emergency_exception_buf(100)
           ##===== Wykrycie kliknięcia przycisku ========##
                #=== Warunek, aby wyczyścić odpowiednią część na wyświetlaczu ==============#
            ...
            self.interaction_przycisk = time.ticks_ms()   # przypisanie aktualnego czasu po kliknięciu przycisku
            ##===== Wybudzenie ze snu ========##
            if self.spac_menu == True : 
                self.flaga_sen_stop_menu = False
            #==================================#
              
        elif (self.Przycisk_2_Pin.value() == 0) and (self.Pin18_state == 1 ):
            self.Pin18_state = 0
            
        self.Przycisk_2_Pin.irq(handler = self.Przycisk_2)
...
#################################### KONIEC funckji Przerwania ####################################################  
    ...
    def przytrzymanie_przycisku(self,wybor_przycisku ,podany_czas): 
        if wybor_przycisku == 1: # Lewy przycisk ( zielony)
